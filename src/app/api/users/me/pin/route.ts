...
import { resolveUserIdToUUID } from '@/lib/db';
...
export const GET = withAuth(async (req: AuthRequest) => {
  ...
  const uuid = await resolveUserIdToUUID(userId);
  const result = await query('SELECT ulaz_pin FROM users WHERE id = $1', [uuid]);
...
export const PUT = withAuth(async (req: AuthRequest) => {
  ...
  await query('UPDATE users SET ulaz_pin = $1 WHERE id = $2', [newPin, uuid]);
